...
class cFO(cPluginParent):
    ...
    def Analyze(self):
        ...
        if len(self.streamname) > 1 and self.streamname[-1] in ['o']:
            self.ran = True
            while len(self.stream) > 0:
                ...
                if code == 0x200:
                    ...
                    if fieldtype == 0x80400101:
                        ...
                        if self.options == '-d':
                            result.append('c:%04x l:%04x t:%08x %s' % (code, length, fieldtype, self.stream[0x1C:0x1C + lengthString]))
                        else:
                            result.append(self.stream[0x1C:0x1C + lengthString])
                    else:
                        if self.options == '-d':
                            result.append('c:%04x l:%04x f:%08x' % (code, length, fieldtype))
                else:
                    if self.options == '-d':
                        result.append('c:%04x l:%04x' % (code, length))
